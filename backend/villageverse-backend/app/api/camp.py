...
from app.api.weather import get_today_weather
...
async def gather_resource(username: str, resource: str, db: AsyncSession = Depends(get_db)):
    ...
    base_amount = random.randint(cfg["min"], cfg["max"]) + (camp.camp_level - 1) * 2

    weather = get_today_weather()
    weather_multiplier = weather["multiplier"] if weather["boosted_resource"] == resource else 1.0

    companion_multiplier = await get_active_bonus(db, camp.player_id, "gather")

    amount = round(base_amount * weather_multiplier * companion_multiplier)
    ...
    return {
        ...
        "weather_boosted": weather_multiplier > 1.0,
